...
type Reel = NonNullable<Awaited<ReturnType<typeof getReelById>>>;

interface SingleReelViewProps {
  reel: Reel;
  currentDbUserId: string | null;
}

export default function SingleReelView({ reel, currentDbUserId }: SingleReelViewProps) {
  ...
  const handleLike = async () => {
    ...
    setLikeCount((count) => (previousLiked ? count - 1 : count + 1));
  ...
  const handleFollow = async () => {
    ...
    try {
      ...
      if (result?.success) {
        toast.success(previousFollowing ? "Unfollowed" : "Following");
      } else {
        setIsFollowing(previousFollowing);
  ...
  const handleDeleteComment = async (commentId: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.error || "Couldn't delete comment");
      }
    } catch {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      toast.error("Direct download blocked, opening video");
        ...
        <div className="relative h-full w-full overflow-hidden bg-black">
          ...
          <div className="absolute bottom-28 right-3 z-30 flex flex-col items-center gap-5">
            ...
            <button
              ...
              aria-label="Comments"
            >
              <MessageCircleIcon className="h-8 w-8 drop-shadow-lg" strokeWidth={1.8} />
              <span className="text-xs font-semibold drop-shadow-md">
                {comments.length}
              </span>
            </button>
